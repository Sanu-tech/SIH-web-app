...
5.  **Return JSON:** Your response must be a JSON object conforming to the provided schema. It should contain a single key, "presentStudentEmails", which is an array of strings. Each string is the email of a student you confidently identified. If no students are identified, return an empty array.`;
    ...
    const response = await aiClient.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
                presentStudentEmails: {
                    type: Type.ARRAY,
                    items: {
                        type: Type.STRING
                    },
                    description: 'An array of strings, where each string is the Email of a student identified with high confidence as present in the classroom photo.'
                }
            }
          },
        },
...
export const generateDailyRoutine = async (profile: StudentProfile, timetable: ScheduledClass[]): Promise<DailyRoutine[]> => {
  ...
  try {
    const response = await aiClient.models.generateContent({
      model: "gemini-2.5-flash",
      contents: `Generate a personalized daily routine for a student with these career goals: ${profile.careerGoals}. Here is their class schedule for today: ${JSON.stringify(timetable)}. Integrate academic work, breaks, and personal development. Be motivational.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.ARRAY,
          items: {
            ...
            propertyOrdering: ["time", "activity", "details", "type"],
          },
        },
      },
    });
